...
template<class K, class V>
inline void HashTable<K, V>::SearchTest(K key)
{
	//generate a hash with provided key & hash function
	int index = m_hash(key);
	bool found = false;
	...
	//check if the given bucket contains the same key
	typename list<pair<K, V>>::iterator i = m_table[index].begin();
	for (i = m_table[index].begin(); i != m_table[index].end() && !found; i++)
	{
		if ((*i).first == key)
		{
			//identical key in this bucket, found!
			found = true;
		}
	}
